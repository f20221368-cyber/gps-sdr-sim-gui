...
%         To get the real and imaginary parts (sine and cosine)
%         cosine_part = np.real(sigCarr)  Cosine carrier
%         sine_part = np.imag(sigCarr)     Sine carrier
    ...
    if acqResults.peakMetric(PRN) > settings.acqThreshold
        ...
        for fineBinIndex = 1 : numOfFineBins
            ...
            %if you acquire on any arbitrary 20 ms sequence of received data, your acquisition results will vary since a data
            % transition can occur equally likely anywhere in that 20 ms given you select a random starting point for your 
            % acquisition. You avoid this whole issue by effectively performing acquisition using a 20 ms sliding correlation 
            % on 40 ms of received signal (assuming you want to maximize processing gain over the full 20 ms available).
