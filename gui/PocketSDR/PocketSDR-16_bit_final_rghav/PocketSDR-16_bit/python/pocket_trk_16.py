...
def read_data(fp, N, IQ, buff, ix):
    if fp == None:
        raw = np.frombuffer(sys.stdin.buffer.read(2 * N * IQ), dtype='int16')
    else:
        raw = np.frombuffer(fp.read(2 * N * IQ), dtype='int16')
...
def update_corr_3d(ax, p, ch, env, toff, tspan):
    ...
    y1 = Yt[len(Yt)//2]
    yl = [y1 + ch.trk.pos[4] / ch.fs * 1.3e3,
          y1 + ch.trk.pos[-1] / ch.fs * 1.3e3]
...
def update_corr_IQ(ax, p, ch, tspan):
    ...
    p[2].set_text('IP=%6.3f\nQP=%6.3f' %
                  (ch.trk.P[-1].real, ch.trk.P[-1].imag))
...
def show_usage():
    print(
        'Usage: pocket_trk.py [-sig sig -prn prn[,...] ...] [-p] [-e] [-toff toff]')
...
if __name__ == '__main__':
    ...
    try:
        for i in range(0, 1000000000):  # 1000000000
