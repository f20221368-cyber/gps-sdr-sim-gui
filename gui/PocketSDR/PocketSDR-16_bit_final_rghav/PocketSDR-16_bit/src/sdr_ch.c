                      ...
                      int len_buff, int ix)
{
    ...
    if (ch->coff >= ch->T)
    {
        ...
        update_tow(ch, -ch->T);
        ch->lock--;
        memmove(ch->trk->P + 1, ch->trk->P, sizeof(sdr_cpx_t) * (SDR_N_HIST - 1));
    }
    else if (ch->coff < 0.0)
    {
        ch->coff += ch->T;
        update_tow(ch, ch->T);
        ch->lock++;
        memmove(ch->trk->P, ch->trk->P + 1, sizeof(sdr_cpx_t) * (SDR_N_HIST - 1));
    }
