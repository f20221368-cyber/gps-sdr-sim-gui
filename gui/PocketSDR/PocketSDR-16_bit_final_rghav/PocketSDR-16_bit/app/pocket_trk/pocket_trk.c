...
#define FFTW_WISDOM "C:\\Users\\Arya\\Downloads\\PocketSDR-master\\app\\pocket_trk\\fftw_wisdom.txt"
...
static int buff_full(sdr_rcv_t *rcv)
{
    ...
    {
        if (rcv->ix + 1 - rcv->ch[i]->ix >= MAX_BUFF)
            return 1;
    }
    return 0;
}
...
static void print_head(sdr_rcv_t *rcv)
{
    char solstr[128];
    int nch = 0;
    for (int i = 0; i < rcv->nch; i++)
    {
        if (rcv->ch[i]->ch->state != STATE_LOCK)
            continue;
        nch++;
    }
    sdr_pvt_solstr(rcv->pvt, solstr);
    printf("\r %-*s BUFF:%3d%% SRCH:%3d LOCK:%3d/%3d\n", NUM_COL - 38,
           solstr, rcv->buff_use, rcv->ich + 1, nch, rcv->nch);
    printf("%3s %2s %4s %5s %3s %8s %4s %-12s %11s %7s %11s %4s %5s %4s "
           "%4s %3s\n",
...
static void print_ch_stat(sdr_ch_t *ch)
{
    ...
    cn0_bar(ch->cn0, bar);
    sync_stat(ch, stat);
    printf("%s%3d %2d %4s %5s %3d %8.2f %4.1f %-13s%11.7f %7.1f %11.1f %s %5d %4d %4d %3d%s\n",
           ESC_COL, ch->no, 1, ch->sat, ch->sig, ch->prn,
...
static int rcv_print_stat(sdr_rcv_t *rcv, int ncol)
{
    int n = 2;
    ...
    for (int i = 0; i < rcv->nch; i++)
    {
        ...
        if (ch->state != STATE_LOCK || ch->lock * ch->T < MIN_LOCK)
            continue;
        print_ch_stat(ch);
        n++;
    }
...
static void *rcv_ch_thread(void *arg)
{
    ...
    while (ch->state)
    {
        for (; ix + 2 * n <= ch->rcv->ix + 1; ix += n)
        {
            ...
            if (ch->ch->nav->stat)
            {
                sdr_pvt_udnav(ch->rcv->pvt, ch->ch);
                ch->ch->nav->stat = 0;
            }
            // update observation data
            sdr_pvt_udobs(ch->rcv->pvt, ch->ix, ch->ch);
...
static rcv_ch_t *rcv_ch_new(const char *sig, int prn, double fs, double fi,
                            const double *dop, sdr_rcv_t *rcv, int if_ch)
{
...
static sdr_rcv_t *rcv_new(char **sigs, const int *prns, int n, double fs,
                          const double *fi, const double *dop, int fmt, const int *IQ)
{
    ...
    rcv->raw = (int16_t *)sdr_malloc(2 * rcv->N * (fmt || IQ[0] == 1 ? 1 : 2));
...
// stop receiver ---------------------------------------------------------------
static void rcv_stop(sdr_rcv_t *rcv)
{
    for (int i = 0; i < rcv->nch; i++)
    {
...
// free receiver ---------------------------------------------------------------
static void rcv_free(sdr_rcv_t *rcv)
{
    for (int i = 0; i < rcv->nch; i++)
    {
...
//     int N = rcv->N * ((rcv->fmt || rcv->IQ[0] == 1) ? 1 : 2) * 2; // 2 * N * IQ

//     printf("%d, %d", i, N);

//     if (fread(rcv->raw, N, 1, fp) < 1) // fp.read
//     {
//         return 0; // end of file
//     }
//     if (rcv->fmt)
//     { // raw SDR device format
//         if (LUT[0][0][0] == 0.0)
//         {
//             gen_LUT(rcv->IQ, LUT);
//         }
...
//         {
//             uint16_t raw = (uint16_t)rcv->raw[j];
...
//         }
//     }
//     else if (rcv->IQ[1] == 1) // IQ == 2
//     {                         // IQ
//         for (int j = 0; j < rcv->N; i++, j += 2)
//         {
//             rcv->buff[0][i][0] = rcv->raw[j];
//             rcv->buff[0][i][1] = rcv->raw[j + 1];
//         }
//     }
...
//     static sdr_cpx_t LUT[2][256] = {{{0}}};
//     int i = rcv->N * (ix % MAX_BUFF);
//     int N = 2 * rcv->N * ((rcv->fmt || rcv->IQ[0] == 1) ? 1 : 2);
...
static void update_buff_use(sdr_rcv_t *rcv)
{
    ...
    for (int i = 0; i < rcv->nch; i++)
    {
        int use = (rcv->ix + 1 - rcv->ch[i]->ix) * 100 / MAX_BUFF + 1;
        if (use > rcv->buff_use)
            rcv->buff_use = use;
    }
}
...
static void rcv_update_srch(sdr_rcv_t *rcv)
{
    ...
    for (int i = 0; i < rcv->nch; i++)
    {
        ...
        if (rcv->ch[rcv->ich]->ch->state != STATE_IDLE)
            continue;
...
// execute receiver ------------------------------------------------------------
static void rcv_exec(sdr_rcv_t *rcv, FILE *fp, double tint, int quiet)
{
    int ncol = 0;
...
int main(int argc, char **argv)
{
    ...
    // file = "C:\\Users\\Arya\\Downloads\\bt_driver\\capture_quad.dat";
    // fs = 2.4 * 1e6;
    // sig = "L1CA";
    // sigs[0] = sig;
    // sigs[1] = sig;
    ...
    // generate new receiver
    sdr_rcv_t *rcv = rcv_new(sigs, prns, nch, fs, fi, dop, fmt, IQ);
