...
int main(int argc, char *argv[])
{
	...
	while ((result=getopt(argc,argv,"e:u:x:g:c:l:o:s:b:L:T:t:d:ipvS:"))!=-1)
	{
		...
		case 'S':
			skyViewMode = TRUE;
			sscanf(optarg,"%lf,%lf,%lf",&skyLLH[0],&skyLLH[1],&skyLLH[2]);
			skyLLH[0] /= R2D;
			skyLLH[1] /= R2D;
			break;
		case 'v':
	...
	if (neph==0)
	{
		fprintf(stderr, "ERROR: No ephemeris data found.\n");
		exit(1);
	}
	...
	if (skyViewMode)
	{
		...
		llh2xyz(skyLLH, skyXYZ);
		
		printf("[");
		for (sv=0; sv<MAX_SAT; sv++)
		{
			// Use the first ephemeris set (index 0) for visibility check
			int status = checkSatVisibility(eph[0][sv], g0, skyXYZ, elvmask, azel);
			if (status != -1) // If ephemeris exists for this SV
			{
				if (!first) printf(",");
				printf("{\"prn\":%d,\"azimuth\":%.1f,\"elevation\":%.1f,\"visible\":%d}", 
					sv + 1, azel[0]*R2D, azel[1]*R2D, status == 1 ? 1 : 0);
				first = 0;
			}
		}
		printf("]\n");
