...
ipcMain.handle('create-empty-bin-file', async () => {
    ...
    try {
        // Create an empty file (0 bytes) natively on disk
        fs.writeFileSync(filePath, ''); 
        return filePath; // Send back path string to update frontend state
    } catch (error) {
        console.error('Failed to create empty file:', error);
        throw error;
    }
});
